...
impl RegisterRequest {
    ...
    pub fn validate(self) -> Result<Self, AppError> {
        ...
        if !is_valid_email(&email) {
            return Err(AppError::BadRequest("邮箱格式不正确"));
        }
        ...
        if self.password.chars().count() < 8 {
            return Err(AppError::BadRequest("密码长度不能少于 8 个字符"));
        }
        ...
        if display_name.is_empty() {
            return Err(AppError::BadRequest("显示名称不能为空"));
        }
        // 显示名称长度不能超过 100 个字符
        if display_name.chars().count() > 100 {
            return Err(AppError::BadRequest("显示名称长度不能超过 100 个字符"));
        }
