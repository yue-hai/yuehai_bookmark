...
/// * `AppJson(payload)`：将 HTTP JSON 请求体反序列化为 RegisterRequest
/// 
/// # Returns
/// * `Result<impl IntoResponse, AppError>`：成功时返回 HTTP 200 和 JSON，失败时返回 HTTP 错误
pub async fn register(State(state): State<AppState>, AppJson(payload): AppJson<RegisterRequest>) -> Result<impl IntoResponse, AppError> {
